...
export function createPluginSecretsHandler(
  ...
  return {
    async resolve(params: PluginSecretsResolveParams): Promise<string> {
      ...
      // ---------------------------------------------------------------
      // Aerolab single-tenant override.
      //
      // Upstream master ships a fail-closed kill switch here
      // (PLUGIN_SECRET_REFS_DISABLED_MESSAGE) until plugin config becomes
      // company-scoped (upstream PAP-2394). That mitigation guards against
      // cross-company secret disclosure in MULTI-tenant deployments.
      //
      // This instance is single-company (Aerolab only), so that disclosure
      // vector does not apply. We restore the direct UUID resolution path
      // (the pre-#5429 behaviour) so plugins like paperclip-plugin-slack can
      // resolve their bot token / signing secret. Remove this override once
      // upstream re-enables company-scoped plugin secret refs.
      // ---------------------------------------------------------------
